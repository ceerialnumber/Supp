...
export default function CreateEventPage({ onSubmit }: CreateEventPageProps) {
  ...
  const handleSubmit = () => {
    ...
    const newEvent = {
      ...
      description: `A ${selectedType} event newly created! Excited to see everyone there.`,
    };
  ...
  return (
    <div className="max-w-6xl mx-auto pb-32 px-6">
      <div className="animate-in fade-in slide-in-from-bottom-6 duration-500">
        <h1 className="text-4xl font-bold text-[#1D72FE] mb-8 mt-4">Memory making</h1>

        {/* Image Upload Area */}
        <div className="relative w-full aspect-[4/3] bg-gray-100 rounded-[40px] flex flex-col items-center justify-center border-2 border-gray-50 shadow-xl mb-10 group cursor-pointer active:scale-[0.99] transition-all">
          <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-md mb-4 text-[#1D72FE]">
            <Plus className="w-8 h-8" />
          </div>
          <span className="text-[#1D72FE] font-bold text-xl uppercase tracking-wide">add pic</span>
        </div>

        {/* Name Input */}
        <div className="mb-8">
          <label className="block text-xl font-bold text-black mb-3 ml-2">Name</label>
          <input 
            type="text" 
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            placeholder="Name this event..."
            className="w-full bg-white rounded-full py-5 px-8 shadow-[0_10px_30px_rgba(0,0,0,0.05)] border border-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-100 transition-all placeholder:text-gray-300 font-medium"
          />
        </div>

        {/* When Section */}
        <div className="mb-8">
          <label className="block text-xl font-bold text-black mb-3 ml-2">When?</label>
            ...
            <div className="relative">
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] uppercase tracking-widest text-[#1D72FE] font-black">Date</span>
                </div>
              </button>
            </div>
            <div className="relative">
          ...
          {showCalendar && (
            <>
              ...
              <motion.div 
                ...
                className="fixed inset-x-6 top-1/2 -translate-y-1/2 bg-white rounded-[40px] shadow-2xl z-[120] overflow-hidden max-w-sm mx-auto"
              >
                {renderCalendar()}
                <button 
                  onClick={() => setShowCalendar(false)}
          ...
          {showTimePicker && (
            <>
              ...
              <motion.div 
                ...
                className="fixed inset-x-0 bottom-0 bg-white rounded-t-[40px] shadow-[0_-10px_40px_rgba(0,0,0,0.1)] z-[120] overflow-hidden max-w-lg mx-auto w-full"
              >
        ...
        {/* Where Section */}
        <div className="mb-10">
          <label className="block text-xl font-bold text-black mb-3 ml-2">Where?</label>
          <input 
            type="text" 
            value={location}
        ...
        {/* Type Section */}
        <div className="mb-12">
          <label className="block text-xl font-bold text-black mb-6 ml-2">Type</label>
          <div className="flex justify-between items-center overflow-x-auto no-scrollbar py-4 px-2 gap-6">
            {TYPES.map((type) => (
              ...
              >
                ...
                <span className={`text-xs font-bold transition-colors ${selectedType === type.id ? 'text-blue-600' : 'text-gray-500'}`}>
                  {type.label}
                </span>
              </button>
            ))}
        ...
        <div className="flex justify-center">
          ...
          >
            ...
            <span className="text-xl font-bold">Add memory</span>
          </motion.button>
        </div>
      </div>
    </div>
